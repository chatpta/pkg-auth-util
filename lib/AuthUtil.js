...
class AuthUtil {
    ...
    createJWT(header, payload, secretKey = this.defaultSecret) {
        ...
        let signature = this.createHmacString(headerPlusPayload, secretKey, header.alg)
    ...
    verifySignatureJWT(jwt, secretKey = this.defaultSecret) {
        ...
        // No jwt object return false
        if (!jwtObject) return false;
        // Header plus payload still url safe
        let headerPlusPayload = jwtObject.header + '.' + jwtObject.payload;
        // Find header object
        let headerObject = JSON.parse(this.base64ToAscii(jwtObject.header));
        // Calculate signatures
        let signatureCalculated = this.createHmacString(headerPlusPayload, secretKey, headerObject.alg);
