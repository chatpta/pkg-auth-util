...
class Middleware {
    constructor(defaultValues = {}) {
        ...
        this.jwtCreator = new JwtCreator(defaultValues);
        this.jwtReader = new JwtReader(defaultValues);
        this.hash = new Hash(defaultValues);
    }
    ...
    async createIncomingUserHash(req, res, next) {
        if ((!!req.incomingUser) && (!!req.incomingUser.password)) {
            const hash = await this.hash.createPasswordHashStoreString(req.incomingUser.password, this.hash.createRandomSalt());
    ...
    createJwtTokenSHA512(req, res, next) {
        if (!req.jwtHeader || !req.jwtPayload) {
            ...
            req.jwtToken = this.jwtCreator.jwtCreateSHA512(req.jwtHeader, req.jwtPayload);
            req.jwtHeader = null;
            req.jwtPayload = null;
            next();
        }
    ...
    async loginUserUsingReqIncomingUserReqDatabaseUser(req, res, next) {
        if ((!!req.incomingUser) &&
            ...
            await this.hash.verifyPasswordHash(req.incomingUser.password, req.databaseUser.hash) &&
    ...
    verifyIncomingJwtTokenSignature(req, res, next) {
        if (!!req.recoveryJwtToken && this.jwtReader.jwtIsSignatureValid(req.recoveryJwtToken)) {
    ...
    async readReqSignatureVerifiedJwtTokenAttachToReqUser(req, res, next) {
        if ((!!req.signatureVerifiedJwtToken)) {
            req.user = this.jwtReader.jwtRead(req.signatureVerifiedJwtToken);
            req.signatureVerifiedJwtToken = null;
            next();
        } else {
            req.user = null;
