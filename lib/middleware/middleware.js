...
class Middleware extends JwtReader {
     ...
     */
    createIncomingUserHash(req, res, next) {
        if ((!!req.incomingUser) && (!!req.incomingUser.password)) {
            const hash = this.createPasswordHashStoreString(req.incomingUser.password, this.createRandomSalt());
    ...
    loginUserUsingReqIncomingUserReqDatabaseUser(req, res, next) {
        if ((!!req.incomingUser) &&
            ...
            this.verifyPasswordHash(req.incomingUser.password, req.databaseUser.hash) &&
     ...
     * @returns {Promise<void>}
     */
    readReqSignatureVerifiedJwtTokenAttachToReqUser(req, res, next) {
